...
class AdminUsuarioView(ft.View):
    def __init__(self, page):
        ...
        for item in self.sidebar.items_top_sidebar + self.sidebar.items_bottom_sidebar:
            item.set_checked(item.text_val == "Usuário")
    ...
    def build_content(self):
        return ft.Container(
            content=ft.Column([
                ft.Text("Meu Perfil de Usuário", size=30, weight=ft.FontWeight.BOLD, color=ft.Colors.GREY_900),
                ft.Divider(color=ft.Colors.GREY_500),
                ft.Text("Configurações da conta do funcionário logado.", color=ft.Colors.GREY_400)
            ], expand=True),
